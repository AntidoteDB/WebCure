...
for (var i in conf.antidote) {
  atdClis.push(
    antidoteClient.connect(
      conf.antidote[i].port,
      conf.antidote[i].host
    )
  );
}
...
async function call_DisconnectNetwork() {
  const { stdout, stderr } = await exec(
    "docker network disconnect application_interdc application_antidote1_1"
  );
...
async function call_ConnectNetwork() {
  const { stdout, stderr } = await exec(
    "docker network connect application_interdc application_antidote1_1"
  );
  console.log("stdout:", stdout);
  console.log("stderr:", stderr);
}
...
function UserApps(calendarId, user, calendar) {
  // Function returning the Set of aIds for the given view
...
for (var i in userSets) {
  userSets[i]
    .read()
    .then(_ =>
      console.log("Connection established to antidote with UserSet: " + _)
    )
...
exports.readAllUserAppos = function(calendarId, user, calendar) {
  //all aIds of an user,calender tuple;
  return UserApps(calendarId, user, calendar).read();
};

exports.getLastCommitTimestamp = function(calendarId) {
  return atdClis[calendarId - 1].getLastCommitTimestamp().toBase64();
};

exports.readAllAppointments = function(calendarId) {
  // all aId-Appointment tuples
  return appMaps[calendarId - 1].read();
};
exports.readAllParticipants = function(calendarId) {
  // all participants
...
exports.addParticipant = function(calendarId, participant) {
  return atdClis[calendarId - 1].update(
    userSets[calendarId - 1].add(participant)
  );
};

exports.setTheTimestamp = function(calendarId, timestamp, update_clock) {
  if (timestamp && timestamp.data && timestamp.data !== "null") {
    timestamp = bytebuffer.fromBase64(timestamp.data);
    console.log(timestamp);
    atdClis[calendarId - 1].monotonicSnapshots = true;
    atdClis[calendarId - 1].setLastCommitTimestamp(timestamp);
    atdClis[calendarId - 1].update_clock = update_clock;
  }
};

exports.syncParticipants = async function(calendarId, updates) {
  let tx = await atdClis[calendarId - 1].startTransaction();
    let set = tx.set("users");
...
exports.setUpdateClock = function(calendarId, flag){
  atdClis[calendarId - 1].update_clock = flag;
}
...
exports.writeNewAppointment = function(calendarId, calendar, app) {
  ...
  return atdClis[calendarId - 1].update([
    appMaps[calendarId - 1]
      .rrmap(id)
      .multiValueRegister("id")
      .set(app.id),
    appMaps[calendarId - 1]
      .rrmap(id)
      .multiValueRegister("title")
      .set(app.title),
    appMaps[calendarId - 1]
      .rrmap(id)
      .multiValueRegister("start")
      .set(app.start),
    appMaps[calendarId - 1]
      .rrmap(id)
      .multiValueRegister("end")
      .set(app.end),
    appMaps[calendarId - 1]
      .rrmap(id)
      .multiValueRegister("allDay")
      .set(app.allDay),
    appMaps[calendarId - 1]
      .rrmap(id)
      .multiValueRegister("description")
      .set(app.description),
    appMaps[calendarId - 1]
      .rrmap(id)
      .set("participants")
      .addAll(app.participants),
    appMaps[calendarId - 1]
      .rrmap(id)
      .multiValueRegister("priority")
      .set(app.priority),
    appMaps[calendarId - 1]
      .rrmap(id)
      .set("comments")
...
exports.updateAppointment = function(
  calendarId,
  aId,
  app,
  calendar,
  ...
  return userSets[calendarId - 1]
    ...
    .then(_ => {
      ...
      let remove = atdClis[calendarId - 1].update(
        appMaps[calendarId - 1]
          .rrmap(aId)
          .set("participants")
          .removeAll(app.removedParticipants)
      );
      remove
        .then(_ => {
          ...
          let add = atdClis[calendarId - 1].update(
            appMaps[calendarId - 1]
              .rrmap(aId)
              .set("participants")
              .addAll(app.addedParticipants)
          );
          add
            .then(_ => {
              ...
              for (let p in app.addedParticipants) {
                atdClis[calendarId - 1].update(
                  UserApps(calendarId, app.addedParticipants[p], calendar).add(
                    aId
                  )
                );
              }
...
exports.deleteAppointment = function(calendarId, aId) {
  return atdClis[calendarId - 1].update(
    appMaps[calendarId - 1].remove(appMaps[calendarId - 1].rrmap(aId))
  );
};
exports.deleteParticipant = function(calendarId, participant) {
  ...
  UserApps(calendarId, participant, "Business")
    .read()
    .then(priv => {
      atdClis[calendarId - 1].update(
        UserApps(calendarId, participant, "Business").removeAll(priv)
      );
    })
    .catch(err =>
      console.log(
        "failed to read Business calendar of " +
          participant +
          ". Maybe it was empty",
        err
      )
      ...
      for (let i in AllApps) {
        if (contains(AllApps[i].participants, participant)) {
          atdClis[calendarId - 1]
            ...
            .then(res =>
              console.log(
                "removed " + participant + "from app.participants " + i
              )
            )
            .catch(err =>
              console.log(
                "failed remove " + participant + " from app.participants " + i,
                err
              )
            );
        }
  ...
  return atdClis[calendarId - 1].update(
    userSets[calendarId - 1].remove(participant)
  );
};
...
function contains(arr, elem) {
  // check, if an element 'elem' is contained in the array 'arr'
  let i = arr.length;
  while (i--) {
    if (arr[i] === elem) {
      return true;
...
function getNewId() {
  // increment the aId counter and return the current one. (so aIds stay unique)
  count++;
