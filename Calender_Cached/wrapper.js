...
exports.editAppointment = function(
  calendarId,
  id,
  app,
  calendar,
  ...
  let result = storage.updateAppointment(
    calendarId,
    id,
    app,
    calendar,
  ...
  result
    .then(_ => {
      console.log(
        "Appointment successfully edited with id: " +
          id +
...
exports.getUpdates = function(calendarId, participant, calendar, res) {
  ...
  let lastCommitTimestamp = storage.getLastCommitTimestamp(calendarId);

  updateObject.lastCommitTimestamp = lastCommitTimestamp;
  ...
  x.then(valuea => {
    ...
    y.then(valueb => {
      ...
      z.then(value => {
        ...
        if (!"length" in value.entries) console.log("fehler gefunden");
        ...
        console.log(
          "GetUpdates: Participants send back to client, readAllAppointments failed",
          err
        );
      ...
      console.log(
        "GetUpdates: Participants send back to client, readAllUserAppos failed",
        err
      );
      res.send(updateObject);
    });
    ...
    console.log(
      "Wrapper.getUpdates: failed to read Updates, readAllParticipants failed",
      err
    )
  );
};
exports.addParticipant = function(calendarId, participant, res) {
  // add new participant to storage
  let result = storage.addParticipant(calendarId, participant);
  let lastCommitTimestamp = storage.getLastCommitTimestamp(calendarId);
  result
    .then(_ => {
      res.send({ result: true, lastCommitTimestamp: lastCommitTimestamp });
    })
    .catch(err => {
...
exports.removeParticipant = function(calendarId, participant, res) {
  ...
  result
    ...
    .catch(err => {
      console.log("wrapper: failed to delete participant " + participant, err),
        res.send({ result: false });
    });
};

exports.syncUsers = async function(calendarId, updates, lastCommitTimestamp, res) {
  storage.setTheTimestamp(calendarId, lastCommitTimestamp, false);
  storage.syncParticipants(calendarId, updates);
  storage.setUpdateClock(calendarId, true);

  res.json({
    status: "OK",
    lastCommitTimestamp: storage.getLastCommitTimestamp(calendarId)
  });
};

function compareAppsWithUserApps(apps, userapps) {
  // calculates join to get necessary appointment structures
  let result = [];
  if (userapps == undefined || apps == undefined) return result;
...
function MapToJSON(map) {
  // eliminates the array structure due to MVRs and ad conflict flag if necessary
  ...
  if (isConflict) {
    // in case of conflict, app covers the conflict flag and the real application structure. else, it only covers the properties
    app.conflict = true;
    app.app = map;
  }
  return app;
...
function isConflicted(app) {
  ...
  if (
    ...
    app.end.length > 1 ||
    app.allDay.length > 1 ||
    app.description.length > 1 ||
    app.priority.length > 1
  )
    conflict = true;
  return conflict;
}
