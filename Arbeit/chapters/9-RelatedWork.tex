...
However, as our reader might have guessed, it is much more interesting, when we have more than one server. Let us assume that now we have two CouchDB servers. Imagine we try to update the \textit{document} once again with the values shown in \lstref*{lst:rwork4}.
...
For example, let us say that it gets written to the first CouchDB server, which gives the response shown in \lstref*{lst:rwork5}.
...
Let us now summarise the advantages WebCure has due to using AntidoteDB and not CouchDB. 
