...
In this chapter, we describe the technologies we used to implement the design we introduced in the \chapref*{Design}. To give an overview, the implementation of the design for WebCure demands the following components:
...
At~\lstref*{lst:tech6} we see an example code of how a cache with a name \textit{my-cache} is created. If the operation is performed successfully, then a promise resolves, and one is going to get access either to a newly created cache or to the one, which existed before the call of \textit{open} method.
