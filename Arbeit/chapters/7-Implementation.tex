...
\begin{lstlisting}[caption={[A function, which synchronises operations with the server when the connection is re-established]A function \textit{pushSetChangesToServer}, triggered every time when a client re-connects to the network.}, label={lst:dev12}]
